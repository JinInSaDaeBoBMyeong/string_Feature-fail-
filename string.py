...
def save_csv(count, target_dir_path, target_df, filename):
    print("[>] CSV 변환 시작")
    target_df.to_csv(str(count) + '.csv', mode ='w')
    
if __name__ == "__main__":
    apk_dir_path = 'C:\\Users\\quddu\\Desktop\\test'
    csv_save_path = 'C:\\Users\\quddu\\Desktop\\csv'
    count = 1
    file_list = file_list_fun(apk_dir_path)
    csv_df = tf_idf(androguard_string(apk_dir_path + '\\' + str(file_list[0])))
    save_csv(count, csv_save_path, csv_df, file_list)
